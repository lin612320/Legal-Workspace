...
2. **法规查询**：关键词检索与向量检索两种模式（篇名/条文号/章节/内容），左列表右正文阅读。**出厂内置日美法库**：日本 2,104 部现行法 + 美国法典 53 个 Title + 美利坚合众国宪法，条文级共 152,940 条（来源 e-Gov / govinfo / constitutioncenter.org）。
...
已实现：法规检索（关键词 + 向量）、模板库、AI 助手（RAG/多会话/置顶悬浮窗）、翻译、待办 + 桌面提醒、专注计时、Excel 导入、手动/自动备份、设置持久化、悬浮球双向联动。
