...
律政工作台是一个面向律师、法务等法律工作者的本地桌面应用，围绕日常办案流程提供法规查询、文书模板、AI 辅助、文书智能体、翻译、待办提醒等一体化工具。**配套「悬浮球助手」桌面常驻小球**，选中文字即可翻译、检索或询问 AI。当前版本 `0.4.0`：安装包**内置日美法律数据库**（15.3 万条条文，装机首启即用），并新增**文书智能体**（端到端任务交付，可导出 .docx）。
...
5. **文书智能体（数字员工）**：合同审查 / 起诉状 / 质证意见三条端到端流水线——模型结构化**制定计划卡**（可编辑），检索类步骤走**自主工具调用循环**（`laws_search / law_by_id / template_load / translate_text` 等，只引用工具真实返回的法条），支持人工确认闸门、任务历史载入续跑与执行日志；交付物实时预览并可**导出 .docx**（记入首页「最近文书」，浏览器版下载 .md）。未配置模型时进入演示模式（预设流水线 + 工具真实执行）。
