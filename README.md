...
> 品牌口径：传播名 **律衡**；工程与历史名 律政工作台 / legal-workbench（0.4.x 安装包与进程标识暂沿用，正式更名随后续版本）。

律衡是一个面向律师、法务等法律工作者的本地桌面应用，围绕日常办案流程提供法规查询、文书模板、AI 辅助、文书智能体、翻译、待办提醒等一体化工具。**配套「悬浮球助手」桌面常驻小球**，选中文字即可翻译、检索或询问 AI。当前版本 `0.4.1`：0.4.0 起安装包**内置日美法律数据库**（15.3 万条条文，装机首启即用）并新增**文书智能体「小衡」**（端到端任务交付，可导出 .docx）；0.4.1 正式采用品牌名"律衡"并为智能体定名人设"小衡"。
...
5. **文书智能体「小衡」（数字员工）**：把合同/案情交给小衡，按流水线交付文书——模型结构化**制定计划卡**（可编辑），检索类步骤走**自主工具调用循环**（`laws_search / law_by_id / template_load / translate_text` 等，只引用工具真实返回的法条），支持人工确认闸门、任务历史载入续跑与执行日志；交付物实时预览并可**导出 .docx**（记入首页「最近文书」，浏览器版下载 .md）。未配置模型时进入演示模式（预设流水线 + 工具真实执行）。
...
- **推送给主应用**：悬浮球可将选中文字直接推送到律政工作台 AI 问答输入框（文书智能体页则自动预填为任务材料）。
...
- 主应用安装包：`npm run tauri build` → `src-tauri/target/release/bundle/nsis/律政工作台_0.4.1_x64-setup.exe`。安装包内嵌悬浮球与法库预置库，装入**版本化子目录**（如 `win-unpacked-0.4.1`），升级安装不会与正在运行的旧悬浮球发生文件占用冲突。
