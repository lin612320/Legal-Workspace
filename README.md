...
> 品牌口径：主品牌 **律衡 · Legal Workbench**（「律衡」为传播名）；0.5.0 起安装包与产品显示名正式采用「律衡」。工程与历史名：律政工作台 / legal-workbench——0.4.x 及更早安装包用「律政工作台_0.4.x」命名；进程标识与数据目录（`legal-workbench.exe`、`com.legalworkbench.app`）保持不变，升级不丢数据。

律衡是一个面向律师、法务等法律工作者的本地桌面应用，围绕日常办案流程提供法规查询、文书模板、AI 辅助、文书智能体、翻译、待办提醒等一体化工具。**配套「悬浮球助手」桌面常驻小球**，选中文字即可翻译、检索或询问 AI。当前版本 `0.5.0`：0.4.0 起安装包**内置日美法律数据库**（15.3 万条条文，装机首启即用）并新增**文书智能体「小衡」**（端到端任务交付，可导出 .docx）；0.4.1 正式采用品牌名"律衡"并为智能体定名人设"小衡"；0.4.2 为文书智能体新增**确定性引用自检**（质检为代码级规则校验，非模型自述）；0.5.0 安装包/产品名正式更名「律衡」（工程名 legal-workbench 不变），并让悬浮球「关联查找法规」真正可用（经主程序在本地 15 万条法库检索并回显命中）。
...
5. **文书智能体「小衡」（数字员工）**：把合同/案情交给小衡，按流水线交付文书——模型结构化**制定计划卡**（可编辑），检索类步骤走**自主工具调用循环**（`laws_search / law_by_id / template_load / translate_text` 等，只引用工具真实返回的法条），支持人工确认闸门、任务历史载入续跑与执行日志；**质检含确定性引用自检**（规则校验：识别正文条文号并逐一与工具返回法条比对，未覆盖引用自动点名提示，非模型自述），交付物实时预览并可**导出 .docx**（记入首页「最近文书」，浏览器版下载 .md）。未配置模型时进入演示模式（预设流水线 + 工具真实执行）。
...
- **推送给主应用**：悬浮球可将选中文字直接推送到律衡 AI 问答输入框（文书智能体页则自动预填为任务材料）。
- **外观可定制**：多套球皮肤与面板主题（内置皮肤选择窗）、悬停对话气泡。
- **随主应用联动**：律衡启动时自动拉起悬浮球；顶栏「🎯 悬浮球」按钮唤起面板；悬浮球也可自动拉起打包版 `legal-workbench.exe`。
...
| [docs/07-真机验收清单.md](docs/07-真机验收清单.md) | 真机验收清单、评审演示关键词对照、中文法库接入方案 |
...
- 主应用安装包：`npm run tauri build` → `src-tauri/target/release/bundle/nsis/律衡_0.5.0_x64-setup.exe`。安装包内嵌悬浮球与法库预置库，装入**版本化子目录**（如 `win-unpacked-0.5.0`），升级安装不会与正在运行的旧悬浮球发生文件占用冲突。
- 悬浮球便携版（独立使用）：在 `floating-ball/` 下 `npm run dist` → `floating-ball/dist/悬浮球助手 1.0.0.exe`。
- 完整交付物统一放入 `发布包/`。
- **升级提示**：升级前请退出正在运行的律衡主程序；托盘中的旧悬浮球可保留（新版主程序会自动定位新版悬浮球，下次重启生效）。
